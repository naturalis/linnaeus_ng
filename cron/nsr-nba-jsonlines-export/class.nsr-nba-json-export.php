<?php
	...
	class taxonJsonExporter
	{
		...
		private function getFilePath() {
            return realpath($this->exportFolder) . DIRECTORY_SEPARATOR . $this->filename;
        }

		private function createFilename()
        {
            return $this->fileNameBase . "--" . $this->fileNameDate . "--" . sprintf( '%02s', $this->filecounter++ ) . '.' . $this->extension;
        }
		...
		private function checkEssentials()
		{
            ...
            if ( !realpath($this->exportFolder)) {
                $b[] = "export folder does not exist";
            }
        ...
        private function writeData()
		{
			...
			$this->feedback("exporting " . count($this->taxa) . " taxa");
			...
			foreach((array)$this->taxa as $key=>$val)
			{
				...
				if ( $this->includeNames )
				{
					...
					$k=0;
					$names=array();
					foreach((array)$n as $name) $names['name__'.($k++)]=$name;
				}
				...
				if ( $this->includeImages )
				{
					...
					$l=0;
					$images=array();
					foreach((array)$c as $image)
					{
						$image['licence']=$this->cleanImageLicence($image['licence']);
						$images['image__'.($l++)]=$image;
					}
				}
