...
{foreach item=v key=k from=$states}
	<tr>
		...
		<td>{$v.label}</td>
		<td title="{t}move up{/t}">{if $k>0}<span class="a" onclick="matrixDoMoveState({$v.id},'u');">&nbsp;&uarr;&nbsp;</span>{/if}</td>
		<td title="{t}move down{/t}">{if $k<$states|@count-1}<span class="a" onclick="matrixDoMoveState({$v.id},'d');">&nbsp;&darr;&nbsp;</span>{/if}</td>
	</tr>
{/foreach}
</table>
<p>
