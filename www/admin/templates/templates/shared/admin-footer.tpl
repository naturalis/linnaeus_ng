...
<div class="git-info">
commit <span title="{$GitVars->commit->hash}">{$GitVars->commit->hash_short}</span>{if $GitVars->commit->hash!=$GitVars->origin_commit_hash}<span title="{t}commit differs from origin{/t}">&deg;</span>{/if} {if $GitVars->git_tags}[{$GitVars->git_tags}] {/if}({$GitVars->commit->date}) ({$GitVars->branch} branch @ {$server_name})
</div>
{/if}
