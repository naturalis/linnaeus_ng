...
<div class="git-info">
commit <span title="{$GitVars->commit->hash}">{$GitVars->commit->hash_short}</span>{if $GitVars->commit->hash!=$GitVars->commit->origin_commit_hash}&deg;{/if} ({$GitVars->commit->date}) ({$GitVars->branch} branch @ {$server_name})
</div>
{/if}
