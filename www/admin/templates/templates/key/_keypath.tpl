...
{section name=i loop=$keyPath}
	...
	{else}
		<i>
			<span class="pseudo-a" onclick="$('#next').val({$keyPath[i].id});$('#nextForm').submit();">{$keyPath[i].title}</span>{if $keyPath[i].choice}<!-- span class="keypath-edit" onclick="$('#id').val({$keyPath[i].id});$('#theForm').submit();">{t}edit{/t}</span -->:
