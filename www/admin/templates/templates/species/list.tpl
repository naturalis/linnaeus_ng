...
{if $buffer==true}
{assign var=buffer value=false}
{else}
{assign var=buffer value=true}
{/if}
{/if}
...
<tr class="taxon-list-row" id="row-{$taxa[i].id}">
	...
	<td>
		...
		<span class="pseudo-a" title="move taxon downward" onclick="$('#scroll').val($(window).scrollTop());$('#id').val({$t});$('#move').val('up');$('#rearrangeForm').submit();">&uarr;</span>
		{else}
		<span class="pseudo-a" title="move taxon upward" onclick="$('#scroll').val($(window).scrollTop());$('#id').val({$t});$('#move').val('down');$('#rearrangeForm').submit();">&darr;</span>
		{/if}
		{/if}
	</td>
	<td
		...
		title="{$languages[j].publish[$t].published} of {$languages[j].publish[$t].total} pages published"
