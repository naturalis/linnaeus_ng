...
<div id="page-main">
	<div id="compare">
		<p>
			...
			<option value="{$v.id}">{$v.taxon}{if $v.is_hybrid==1} {$session.project.hybrid_marker}{/if}</option>
			{/foreach}
			</select>
		</p>
		<p>
			...
			<option disabled="disabled" selected="selected" value="">{t}select a taxon{/t}</option>
			{foreach from=$taxa key=k item=v}
			<option value="{$v.id}">{$v.taxon}{if $v.is_hybrid==1} {$session.project.hybrid_marker}{/if}
</option>
			{/foreach}
			</select>
		</p>
