...
{foreach $content.classification v k classification}
    <a href="../species/nsr_taxon.php?id={$v.id}">{$v.name}</a>{if $v.rank_label} [{$v.rank_label}]{/if}
    {* if $smarty.foreach.classification.last || $v.is_empty==1}
        {$v.label}
    {else}
        <a href="../species/nsr_taxon.php/nsr_taxon.php?id={$v.id}">{$v.name}</a>
    {/if *}
    <br />
{/foreach}
</p>
...
{foreach $content.taxonlist v k list}
    <a href="../species/nsr_taxon.php?id={$v.id}">{$v.label}</a> {if $v.rank_label}[{$v.rank_label}]{/if}
