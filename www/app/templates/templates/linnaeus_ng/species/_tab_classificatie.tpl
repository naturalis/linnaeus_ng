<div id="classification">
	...
	{foreach $content.classification v k classification}
		...
		{* if $smarty.foreach.classification.last || $v.is_empty==1}
			{$v.label}
		{else}
			<a href="../{if $v.lower_taxon==1}species/nsr_taxon.php{else}highertaxa/taxon.php{/if}/nsr_taxon.php?id={$v.id}">{$v.name}</a>
		{/if *}
		<br />
	{/foreach}
	</p>
	...
	{foreach $content.taxonlist v k list}
		<a href="../{if $v.lower_taxon==1}species/nsr_taxon.php{else}highertaxa/taxon.php{/if}?id={$v.id}">{$v.label}</a> {if $v.rank_label}[{$v.rank_label}]{/if}
