...
{$c}Resultaten 1-{$results.data|@count} van {$results.count}{if $searchHR} voor '{$searchHR|@escape:quotes}'{/if}{$c}{$b}{$c}wetenschappelijke naam{$c}{$a}{$c}nederlandse naam{$c}{$a}{$c}status voorkomen{$c}{$a}{$c}barcode exemplaren{$c}{$a}{$c}link naar soortenregister{$c}{$a}{$c}link naar afbeelding{$c}{$b}{foreach from=$results.data item=v}
