		...
		$l1 = $this->models->Glossary->_get(
			array(
				...
				'columns' => '
					...
					term as label,
					"glossary" as source,
					concat("../glossary/term.php?id=",id) as url'
			)
		);
		...
		$l2 = $this->models->GlossarySynonym->_get(
			array(
				...
				'columns' => '
					...
					concat("../glossary/term.php?id=",glossary_id) as url'
			)
		);
		...
		$l = $this->models->Literature->_get(
			array('id' =>
				'select
					...
					concat("../literature/reference.php?id=",id) as url
				from %table%
				where
		...
		$cfm = $this->models->ContentFreeModule->_get(
			array(
				...
				'columns' => 'distinct page_id as id, topic as label,
					concat("../module/index.php?page=",page_id,"&modId=",module_id) as url,
					module_id'
			)
		);
