...
/* 09-03-13: export van exoten inclusief hun paspoort voor Hannco Bakker 
 * 

Stuur naar een template met de volgende code:
...
{foreach from=$fields item=v}{$c}{$v}{$c}{$a}{/foreach}{$b}{foreach from=$results.data item=row}{foreach from=$row item=v}{$c}{$v}{$c}{$a}{/foreach}{$b}{/foreach}
	...
	public function searchExtendedAction()
	{
		...
		{
			$search['limit']=3000;
			$template='export_search_extended_blah';
			$this->smarty->assign('csvExportSettings',$this->csvExportSettings);
			$this->smarty->assign('url_taxon_detail',"http://". $_SERVER['HTTP_HOST'].'/nsr/concept/');
		...
		require_once dirname(__FILE__) . '/../models/TraitsModel.php';
		$this->models->TraitsModel = new TraitsModel;
		require_once dirname(__FILE__) . '/../models/SpeciesModel.php';
		$this->models->SpeciesModel = new SpeciesModel;
	
		// Get unique fields in passport
		foreach ($results['data'] as $row) {
			$traits = $this->_getTraits(array(
					'group' => 1,
					'project' => $this->getCurrentProjectId(),
					'language' => $this->getCurrentLanguageId(),
					'taxon' => $row['id']
			));
			foreach ($traits['data'] as $data) {
				$fields[$data['trait']['name']] = '';
			}
		}
	
		// Insert data from passport query into $result
		foreach ($results['data'] as $i => $row) {
			// Merge empty fields with $result first
			$results['data'][$i] = array_merge($row, $fields);
			// Add traits
			$traits = $this->_getTraits(array(
					'group' => 1,
					'project' => $this->getCurrentProjectId(),
			...
			foreach ($traits['data'] as $data) {
				// Concatenate if multiple values
				if (count($data['values']) == 1) {
					$value = $data['values'][0]['value_start'];
				} else {
					$value = implode('; ', array_column($data['values'], 'value_start'));
				}
				// Add to results
				$results['data'][$i][$data['trait']['name']] = $value;
			}
			// Add habitat from NSR
			$presence = $this->models->SpeciesModel->getPresenceDataNsr(array(
					'taxonId' => $row['id'],
					'projectId' => $this->getCurrentProjectId(),
					'languageId' => $this->getCurrentLanguageId()
			));
			$results['data'][$i]['habitat_nsr'] = $presence[0]['habitat_label'];
		}
	
		// Add line with fields
		foreach ($results['data'][0] as $k => $v) {
			$firstLine[] = $k;
		}
	
		$this->smarty->assign('searchHR',$this->makeReadableQueryString());
		$this->smarty->assign('results',$results);
		$this->smarty->assign('fields',$firstLine);
		$this->smarty->assign('search_presence_help_url',$this->_search_presence_help_url);
	
		$this->printPage($template);
	}
	
	// Kloon van functie in Traits controller minus referenties
	private function _getTraits($p)
	{
		$project=  $p['project'];
		$language = $p['language'];
		$taxon= $p['taxon'];
