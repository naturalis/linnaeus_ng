...
//q(($c[$key+1]));
	...
	public function stateAction()
	{
		...
		if ($this->rHasId()) {
			...
			if (isset($state)) $this->requestData['char'] = $state['characteristic_id'];
    ...
    public function ajaxInterfaceAction ()
    {
        ...
        if ($this->requestData['action'] == 'remove_taxon') {
			...
			foreach((array)$d as $val)  if ($val['id']!=$this->getCurrentMatrixId()) array_push($matrices,$val);			
	...
	private function getCharacteristicStateLabels($id)
	{
		...
		return array('label' => $cls[$this->getDefaultProjectLanguage()]['label'], 'labels' => $cls);
	...
	private function cleanUpEmptyVariables()
	{
		...
		*/
		...
		$cs = $this->models->CharacteristicState->_get(
			array('id' => array('project_id' => $this->getCurrentProjectId()))
		);
