...
class ImportL2Controller extends Controller
{
                ...
                if ($cfm === true) {
                    ...
                    if (!empty($image)) {
                        ...
                        if ($this->cRename($_SESSION['admin']['system']['import']['imagePath'] . $image, $paths['project_media'] . $image)) {
                            ...
                            $this->models->FreeModulePage->update(array(
                                'image' => $image
                            ), array(
                                'project_id' => $this->getNewProjectId(), 
                                'id' => $newPageId
    ...
    {
        $this->models->GeodataType->save(array(
            'id' => null, 
            'project_id' => $this->getNewProjectId(), 
            'colour' => 'FFFFFF'
        ));
    ...
    private function saveL2Map ($p)
    {
        ...
        if ($this->cRename($_SESSION['admin']['system']['import']['imagePath'] . $mapImageName, $paths['project_media_l2_maps'] . $mapImageName)) {
            ...
            $this->models->L2Map->update(array(
                'image' => $mapImageName
            ), array(
    ...
    private function fixEmbeddedLinksAndMedia ()
    {
        ...
        $d = $this->models->ContentKeystep->_get(array(
            'id' => array(
                'project_id' => $this->getNewProjectId()
            )
        ));
        ...
        $d = $this->models->ChoiceContentKeystep->_get(array(
            'id' => array(
                'project_id' => $this->getNewProjectId()
            )
        ));
        ...
        $d = $this->models->ContentFreeModule->_get(array(
            'id' => array(
                'project_id' => $this->getNewProjectId()
            )
        ));
