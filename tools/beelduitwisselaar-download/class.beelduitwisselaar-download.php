<?php
	...
	class beeldbankDownloader
	{
		...
		private function requestNsrImagePages()
		{
			if ( $this->number_saved>0 )
			{
				if ( $this->doRequestNsrImageSearchPages )
				{
					...
					for ($i=0;$i<=10;$i++)
					{
						$d=file_get_contents( sprintf($this->urlNsrImageSearch,$i ) );
					}
				}
				else
				{
		...
		private function writeDeleteList()
		{
			...
			{
				$b=array_map( function( $val ) { return $val->filename; } , array_filter( $this->newImages, function( $val ) { return empty($val->_status); } ) );
