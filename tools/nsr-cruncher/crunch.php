...
die('nsr data file cruncher dies at line 3');
	...
	$qNames			= "insert into nsr_taxon_namen (concept_id,language_id,Label,Label_type,hasLanguage_resource,isNameOf_resource,
						typeOfName,hasNameAuthor,hasAuthorship,hasAuthorshipYear,hasUninomial,hasSpecificEpithet) values (%s,%s,'%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')";
	...
	while (!feof($file)) {
		...
		if ($line=='</rdf:Description>') {
			if (isset($Label) && isset($hasLanguage_resource)) {
				...
				$q = sprintf($qNames, 
					...
					isset($isNameOf_resource) ? mysql_real_escape_string($isNameOf_resource) : 'null',
					isset($typeOfName) ? mysql_real_escape_string($typeOfName) : 'null',
					isset($hasNameAuthor) ? mysql_real_escape_string($hasNameAuthor) : 'null',
					isset($hasAuthorship) ? mysql_real_escape_string($hasAuthorship) : 'null',
					isset($hasAuthorshipYear) ? mysql_real_escape_string($hasAuthorshipYear) : 'null',
					isset($hasUninomial) ? mysql_real_escape_string($hasUninomial) : 'null',
					isset($hasSpecificEpithet) ? mysql_real_escape_string($hasSpecificEpithet) : 'null'
				);
			...
			$rec = false;
			$concept_id = $language_id = $Label = $Label_type = $hasLanguage_resource = $typeOfName = 
			$isNameOf_resource = $hasNameAuthor = $hasAuthorship = $hasAuthorshipYear = $hasUninomial = $hasSpecificEpithet = null;
		...
		if ($rec) {
			...
			if (stripos($line,'<rna:hasNameAuthor>')!==false) {
				$hasNameAuthor = strip_tags($line);
			} else
			if (stripos($line,'<rna:hasAuthorship>')!==false) {
				$hasAuthorship = strip_tags($line);
			} else
			if (stripos($line,'<rna:hasAuthorshipYear>')!==false) {
				$hasAuthorshipYear = strip_tags($line);
			} else
			if (stripos($line,'<rna:hasUninomial>')!==false) {
				$hasUninomial = strip_tags($line);
			} else
			if (stripos($line,'<rna:hasSpecificEpithet>')!==false) {
				$hasSpecificEpithet = strip_tags($line);
			} 
...
CREATE TABLE IF NOT EXISTS nsr_taxon_namen (		// corresponds to "NSR taxon namen.rdf.xml"
  ...
  hasNameAuthor varchar(96) DEFAULT NULL,
  hasAuthorship varchar(128) DEFAULT NULL,
  hasAuthorshipYear varchar(16) DEFAULT NULL,
  hasUninomial varchar(64) DEFAULT NULL,
  hasSpecificEpithet varchar(64) DEFAULT NULL,  
  PRIMARY KEY (id)
) ENGINE=MyISAM  DEFAULT CHARSET=utf8;
